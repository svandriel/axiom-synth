...
```text
Keyboard events (Keyboard.vue) -> AxiomSynth.noteOn/noteOff -> Synth.voiceManager -> VoiceManager.voicePool (16 AxiomVoice)
...
| `Synth`               | Destroyed-state guard and delegation of note lifecycle calls to `VoiceManager`                                                                                                        | Voice-pool bookkeeping, allocation policy, Axiom-specific sources/configs | `packages/audio-engine/src/engine/synth.ts`                                                          |
| `VoiceManager`        | Voice pool, `noteToVoiceMap`, note retriggering, voice stealing/choke, `allNotesOff`, and destruction of allocated voices                                                             | Axiom-specific sources/configs, per-voice audio graph                     | `packages/audio-engine/src/engine/voice-manager.ts`                                                  |
| `AxiomSynth`          | Axiom shared sources, configs, setters; builds `AxiomVoiceConfig` + `output` bus; `createVoice()`                                                                                     | Context/master chain, voice-pool bookkeeping, allocation policy           | `packages/axiom-synth/src/axiom-synth.ts`                                                            |
...
| Voice manager delegation                      | `Synth` forwards note lifecycle operations to its `VoiceManager`; `VoiceManager` lazily creates and owns the fixed voice pool                                                           | Separates generic allocation policy from synth-specific voice construction and parameter configuration          |
...
- Voice-stealing uses a `Map<note, Voice>` plus a fixed 16-pool (`packages/audio-engine/src/engine/voice-manager.ts`); when the pool is exhausted it always steals a released voice first (the one closest to silence wins), falling back to the oldest triggered (held) voice only when every busy voice is held, and always chokes the victim over 3ms (`startDelay = 0.003`). Chord-heavy playing (>>16 notes) at 3ms steal latency may feel laggy.
...
- `packages/audio-engine/src/engine/synth.ts` (destroyed-state guard and delegation to `VoiceManager`)
- `packages/audio-engine/src/engine/voice-manager.ts` (voice pool, note routing, stealing/choke, `allNotesOff`, voice destruction)
