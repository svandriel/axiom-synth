...
| Functions/methods  | camelCase; `void`-returning event/key handlers named `on*`/`handle*`                                                                                  | `onPointerDown`, `noteOn`, `createOscillators` | `app/src/components/Knob.vue`, `packages/audio-engine/src/engine/voice.ts`                                                                            |
