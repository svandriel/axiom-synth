...
- Alias vs relative import policy: no path aliases configured; cross-package imports use the workspace package name — the app imports the engine and the synth via `@axiom/audio-engine` and `@axiom/axiom-synth` (e.g. `import { AudioEngine } from '@axiom/audio-engine'`, `import { AxiomSynth } from '@axiom/axiom-synth'`, `import type { EnvelopeConfig } from '@axiom/audio-engine'`); the synth imports engine building blocks via `@axiom/audio-engine` (e.g. `import { Synth, Voice } from '@axiom/audio-engine'`). App-internal imports are relative (`../types`, `./utils/observable`).
- Public exports/barrel policy: `@axiom/audio-engine` exposes source directly via `exports` → `./src/index.ts` (no build artifacts); the package barrel exports the host engine, building-block units, and the abstract `Voice`/`Synth` bases (`AudioEngine`, `Synth`, `Voice`, `Oscillator`, `Filter`, `Envelope`, `Lfo`, `Waveshaper`, `WaveshaperCurve`, `ModulationRouter`, `FilterResonance`, `Meter`, `Analyser`, `helpers`, `constants`) plus the config types (`EnvelopeConfig`, `FilterConfig`, `OscillatorConfig`, `FixedArray`, `WaveFormType`, `WaveshaperConfig`, `WaveshaperType`, LFO config types). `Observable` (`packages/audio-engine/src/utils/observable.ts`) was internal and is now exported from the barrel. `@axiom/axiom-synth` exports `AxiomSynth`/`AxiomVoice` by explicit name from `packages/axiom-synth/src/index.ts`; `AxiomVoiceConfig` stays internal (per-voice plumbing between `AxiomSynth` and `AxiomVoice`, not public API). In-app barrels: `app/src/types/index.ts` re-exports app types (`NumericKeys`), `app/src/utils/index.ts` barrels display helpers and key-map. App components import these via the barrels (`../types`, `../utils`) rather than the individual files. Files sometimes import with explicit extension (`from '../composables/use-audio-context.ts'`) and sometimes without (`from '../utils/observable'`) — **inconsistent across the codebase**, both compile.
...
- Logging style and required context fields: `console.log` everywhere (`Synth`/`Voice` note lifecycle: per-note `noteOn`/`noteOff` timestamped and voice-keyed as `Voice <id>: noteOn(...)`, `connecting`/`disconnecting` markers when a voice joins/leaves the mixer, `allNotesOff`, voice-steal warnings, `AudioContext was closed`). No structured logger, no log levels, no redaction framework.
...
- `packages/audio-engine/package.json` (`exports` → `./src/index.ts`), `packages/audio-engine/src/index.ts` (barrel); likewise `packages/axiom-synth/package.json` + `packages/axiom-synth/src/index.ts` (barrel)
